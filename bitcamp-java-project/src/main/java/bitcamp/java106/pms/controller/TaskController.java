...
public class TaskController {
    ...
    private void onTaskAdd(final Team team) {
        Task task = new Task(team); //특정 팀에 소속되지 않은 task를 만들지 못하도록  기본 생성자를 없애서 제약.
        ...
        taskDao.insert(task); //woker는 null 상태로 
    }
    ...
    private void onTaskState(final Team team) {
        ...
        System.out.println("상태를 변경할 작업의 번호? ");
        int taskNo = Integer.parseInt(keyScan.nextLine());
        ...
        System.out.printf("'%s'의 작업의 상태 : %s\n", task.getTitle(), getStateLabel(taskNo));
        
        System.out.println("변경할 상태? (0: 작업 대기, 1: 작업중, 9: 작업완료)");
