...
테이블 DB 객체(테이블, 뷰, 함수, 트리거 등)를 생성, 변경, 삭제하는 SQL 명령이다.
...
> create table test1(
  c1 int, 
  c2 float, 
  c3 numeric(6,2), /* 소수점 자릿수를 지정하면 부동소수점으로 사용 - 4자리 정수 2자리 소수점*/
  c4 numeric /* 소수점  이하를 지정하지 않으면 int와 같다. - 소수점 이하 반올림하고 짜름*/
  );
...
- 크기가 가변적이라서 데이터 위치를 찾을 때 시간이 오래 걸린다.
...
DBMS 중에는 고정 크기인 컬럼의 값을 비교할 때 빈자리까지 일치해야만 검사하는 경우도 있다.
즉 c1='abc'에서는 데이터를 찾지 못하고, c1='abc  '여야만 데이터를 찾는 경우가 있다.
그러나 mysql은 고정크기 칼럼이더라도 빈자리를 무시하고 데이터를 찾는다.
...
/* 번호는 중복되지 않았지만, name과 age 값이 중복되는 경우를 막을 수 없다*/
...
> create table test1(
  ...
  eng int,
  math int,
  constraint test1_uk unique (name, age) /* uk -unique key */
  );
...
alter table test1
  modify column no int not null auto_increment; /* 그 후 auto_increment를 지정한다. */
```
