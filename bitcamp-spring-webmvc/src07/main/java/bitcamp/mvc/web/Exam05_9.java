...
public class Exam05_9 {
    ...
    @ResponseBody
    public String m1(
            // application/x-222-form-urlencoded 형식으로 값이 넘어올 때는 
            // 다음과 같이 각각의 값을 개별 변수로 받을 수 있다.
            String name, int age, String tel) {
        return String.format("m1() :name=%s, age=%s, tel=%s", name, age, tel);
    }
    ...
    @ResponseBody
    public String m2(
            // text/plain, text/csv, application/json처럼
            // 통으로 값이 넘어올 때는 개별 변수로 받을 수 없다.
            // 통으로 받아야 한다. 변수명은 상관 없다.
    ...
    public String m4(
            @RequestBody String content) {
        // google-gson 라이브러리를 사용해서 JSON 문자열을 자바 객체로 변환시킨다.
    ...
    public String m5(
        ...
        return String.format("m5() : no=%d, title=%s, content=%s, cteatedDate=%s",
                board.getNo(), 
                board.getTitle(), 
                board.getContent(), 
